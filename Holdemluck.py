...
def calc_equity_exhaustive(hero_cards, opp_cards, board_cards_str, max_samples=10000):
    ...
    used_cards = set(
        hero_cards + opp_cards + convert_str_cards_to_treys(board_cards_str)
    )
...
def filter_hands_and_compute_equity(hands_text):
    ...
    for hand in hands:
        ...
        try:
            ...
            if (
                len(hero_cards_treys) != 2
                or len(opp_cards_treys) != 2
                or len(board_cards_str) > 5
            ):
                continue

            equity = calc_equity_exhaustive(
                hero_cards_treys, opp_cards_treys, board_cards_str
            )
        ...
        allinEV = (
            win_value * equity[0]
            + tie_value * equity[1]
            + lose_value * (1 - equity[0] - equity[1])
        )
...
def process_all_txt_in_folder(folder_path):
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith(".txt"):
            ...
            if result[1] != 0.0:
                print(
                    f"Processed {filename}: Luck = {result[0]:.2f}$, Buy-in = {result[1]:.2f}$"
                )
            else:
